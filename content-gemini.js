// =====================================================
// GEMINI CONTENT SCRIPT - AUTO-INJECT PROMPTS
// =====================================================

(function() {
  'use strict';
  
  console.log('[COGNESION] Gemini content script loaded');
  ...
  async function checkForPendingPrompt() {
    try {
      ...
      if (result.pendingPrompt && result.pendingPromptTarget === 'gemini') {
        console.log('[COGNESION] Found pending prompt for Gemini');
        
        // Wait for input field (Gemini uses a contenteditable div or textarea)
        waitForElement('rich-textarea .ql-editor, textarea[placeholder*="Enter"], [contenteditable="true"]', (inputField) => {
  ...
  function waitForElement(selector, callback, maxWait = 10000) {
    ...
    const checkInterval = setInterval(() => {
      ...
      if (element) {
        ...
        clearInterval(checkInterval);
        console.warn('[COGNESION] Timeout waiting for Gemini input field');
      }
    }, 100);
  }
  ...
  function injectPrompt(inputField, prompt) {
    try {
      ...
      console.log('[COGNESION] Prompt injected into Gemini');
