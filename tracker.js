...
function persistAdminBypassToken(token) {
  ...
  try {
    ...
    // Ignore localStorage failures and let the normal auth flow continue.
  }
}
...
function renderAuthGate(message = 'Sign in from the top-right card to load your saved boards, tracked topics, and Morning Brief workspace.') {
  ...
  gate.innerHTML = `
    <div class="t-gate-box">
      ...
      <div class="t-gate-divider"><span>or</span></div>
      <div class="t-gate-admin-label">Admin pressure-test portal</div>
      <form class="t-auth-inline-form t-gate-admin-form" id="t-gate-admin-form" style="margin-bottom:16px;">
        <input
          id="t-gate-admin-token"
          class="t-auth-inline-input"
          type="text"
          autocomplete="off"
          autocapitalize="off"
        ...
        />
        <button type="submit" class="t-auth-inline-button t-auth-inline-button-secondary" ${AUTH_STATE.busy ? 'disabled' : ''}>
  ...
  gate.querySelector('#t-gate-admin-form')?.addEventListener('submit', async (event) => {
    event.preventDefault();
    const token = gate.querySelector('#t-gate-admin-token')?.value?.trim() || '';
...
async function activateAdminBypass(token) {
  ...
  if (!normalizedToken) {
    AUTH_STATE.error = 'Paste the admin pressure-test token to unlock Topics.';
    AUTH_STATE.note = '';
    renderAuthShell();
    renderAuthGate();
    return false;
...
async function signOutWebUser() {
  if (AUTH_STATE.session?.adminBypass) {
      ...
      <div class="t-hd-box t-hd-actions-box">
        <div class="t-detail-actions">
          <div class="t-detail-actions-top">
            ...
            <a class="t-action-btn t-detail-nav-btn" href="/command">${tl('btn_home')}</a>
          </div>
...
function renderLockedState() {
  ...
  gate.innerHTML = `
    <div class="t-gate-box">
      ...
      <div class="t-gate-actions">
        <a class="t-gate-btn-primary" href="/command">← Back to Command</a>
